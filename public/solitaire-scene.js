...
function paint(light) {
  document.getElementById("solitaire-room").src =
    `assets/props/salon-game-table-${light}-v2.webp`;
  document.getElementById("solitaire-room").hidden = false;
  document.documentElement.dataset.salonLight = light;
}
...
if (!explicit) {
  fetch("/api/world", {
    ...
    .then((world) => {
      if (
        world?.version === "manor-world-v1" &&
        ["day", "night"].includes(world.light)
      )
        paint(world.light);
    })
    .catch(() => {});
